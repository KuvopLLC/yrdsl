...
describe('blog build', () => {
  test('produces a complete output tree', () => {
    ...
    // Index HTML has the post list and at least one post card. Don't
    // assert title text — that's editorial and changes whenever a post
    // is renamed.
    const index = readFileSync(join(HERE, 'index.html'), 'utf8');
    expect(index).toContain('class="post-list"');
    expect(index).toContain('class="post-card-title"');
